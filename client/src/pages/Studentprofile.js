...
export default function StudentProfile() {
  ...
  // Keep preview in sync if user profilePhoto changes
  useEffect(() => {
    if (user?.profilePhoto) {
      setPhotoPreview(`http://127.0.0.1:5000/uploads/${user.profilePhoto}`);
    }
  }, [user?.profilePhoto]);
  ...
  const handlePhotoUpload = async () => {
    ...
    try {
      ...
      const res = await API.post('/auth/upload-photo', fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      // ✅ KEY FIX: update auth context AND localStorage with the new profilePhoto
      const newFilename = res.data.filename;
      const token = localStorage.getItem('fh_token');
      const updatedUser = { ...user, profilePhoto: newFilename };

      // Save updated user back to localStorage so it persists after logout/login
  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      ...
      // ✅ KEY FIX: preserve profilePhoto when updating profile
      const updatedUser = {
        ...user,
        name: updated.name,
  ...
  const pw = pwStrength();
  const ini = user?.name?.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || 'S';
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: 20, maxWidth: 860 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="card" style={{ textAlign: 'center' }}>
            <div style={{ marginBottom: 14 }}>
              {photoPreview ? (
                <img src={photoPreview} alt="Profile"
                  style={{ width: 96, height: 96, borderRadius: '50%', objectFit: 'cover', border: '3px solid var(--success)', margin: '0 auto', display: 'block' }}
                  onError={() => setPhotoPreview(null)} />
              ) : (
                <div style={{ width: 96, height: 96, borderRadius: '50%', background: 'rgba(34,197,94,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '1.8rem', color: 'var(--success)', margin: '0 auto', border: '3px solid var(--border)' }}>
                  {ini}
                </div>
              )}
            </div>
            <div style={{ fontWeight: 700, fontSize: '1rem', marginBottom: 4 }}>{user?.name}</div>
            <span style={{ padding: '2px 10px', borderRadius: 20, fontSize: '.68rem', fontWeight: 600, background: 'rgba(34,197,94,.15)', color: 'var(--success)' }}>
              🎓 Student
            </span>
            {user?.department && (
              <div style={{ fontSize: '.75rem', color: 'var(--text3)', marginTop: 6, marginBottom: 4 }}>{user.department}</div>
            )}
            <div style={{ fontSize: '.72rem', color: 'var(--text3)', marginBottom: 14, wordBreak: 'break-all' }}>{user?.email}</div>
            ...
            {photoFile && (
              <>
                <div style={{ fontSize: '.68rem', color: 'var(--text3)', marginBottom: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {photoFile.name}
                </div>
          ...
          <div className="card">
            ...
            {[
              ...
              <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid var(--surface2)', fontSize: '.8rem' }}>
                ...
                <span style={{ color: 'var(--text2)', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textAlign: 'right' }}>{v}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          {activeTab === 'profile' && (
            <div className="card">
              ...
              <form onSubmit={handleProfileSave}>
                ...
                <div style={{ marginTop: 16, padding: '12px 14px', background: 'rgba(79,142,247,.06)', border: '1px solid rgba(79,142,247,.2)', borderRadius: 8, fontSize: '.8rem', color: 'var(--text2)', marginBottom: 16 }}>
                  💡 Your name will be shown when you submit reviews (unless you choose anonymous).
                </div>
                ...
                <div style={{ maxWidth: 400 }}>
                  <div className="form-group" style={{ position: 'relative' }}>
                    ...
                    <input type={showPw ? 'text' : 'password'} value={pwForm.currentPassword}
                      ...
                      placeholder="Enter current password" style={{ paddingRight: 42 }} required />
